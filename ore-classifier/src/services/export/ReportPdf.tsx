...
const styles = StyleSheet.create({
  ...
  disclaimer: { fontSize: 8, color: '#999', marginBottom: 16 },
  ...
  thumbGrid: { flexDirection: 'row', flexWrap: 'wrap', marginTop: 8 },
  thumb: { width: 150, marginRight: 10, marginBottom: 10 },
  thumbImg: { width: 150, height: 95, objectFit: 'cover', marginBottom: 2, borderRadius: 4 },
  thumbLabel: { fontSize: 8, color: '#555' },
});
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.meta}>
          Месторождение: {deposit?.name ?? '—'} · Автор: {experiment.author} · Дата формирования:{' '}
          {new Date().toLocaleDateString('ru-RU')}
        </Text>
        <Text style={styles.disclaimer}>Внутренняя справка. Не для передачи третьим лицам.</Text>
        ...
        <Text style={styles.sectionTitle}>Метрики по кадрам</Text>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerCell}>Крупные</Text>
        ...
        <Text style={styles.sectionTitle}>Иллюстрации</Text>
        <View style={styles.thumbGrid}>
          {includedFrames.map(
            (f) =>
              frameThumbnails[f.id] && (
                <View style={styles.thumb} key={f.id} wrap={false}>
                  <PdfImage src={frameThumbnails[f.id]} style={styles.thumbImg} />
