...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="KEY=VALUE",
        help="Pass overrides to every training job, e.g. --set training.batch_size=16",
    )
...
def main() -> None:
    ...
    def reap_finished() -> None:
        nonlocal running
        still: list[tuple[subprocess.Popen, str, str, Path]] = []
        for proc, name, gpu_id, log_path in running:
            code = proc.poll()
            if code is None:
                still.append((proc, name, gpu_id, log_path))
                continue
            if code == 0:
    ...
    def gpu_slots_in_use() -> dict[str, int]:
        ...
        for _proc, _name, gpu_id, _log in running:
            counts[gpu_id] += 1
        return counts
    ...
    while queue or running:
        ...
        for gpu_id in gpu_ids:
            while queue and counts[gpu_id] < args.max_jobs and len(running) < max_parallel:
                ...
                proc = launch(cfg_path, gpu_id, log_path, args.set)
                running.append((proc, name, gpu_id, log_path))
                counts[gpu_id] += 1
                started_any = True
